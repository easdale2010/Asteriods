...
namespace asteroids
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        Viewport normview, viewport1, viewport2;
        ...
        int ballsinplay = 3;
        const float ballsizeratio = 0.02f;
        const float miniballsize = 0.09f;
        ...
        protected override void Initialize()
        {
            ...
            normview = graphics.GraphicsDevice.Viewport;
            viewport1 = normview;
            viewport1.Width = ((viewport1.Width - 60) / 2);
            viewport1.X = 20;
            viewport1.Y = 60;
            viewport2 = viewport1;
            viewport2.X += viewport1.Width + 20;
        ...
        void resetlevel()
        {
            ...
            for (int i = 0; i < ballsinplay; i++)
            {
                ...
                do
                {
                    balls[i].position = new Vector3(randomiser.Next(backgscroll.gamewidth - balls[i].rect.Width) + balls[i].rect.Width / 2,
                        randomiser.Next(backgscroll.gameheight - balls[i].rect.Height) + balls[i].rect.Height / 2, 0);
        ...
        int checkcollision(int ballnumber)
        {
            ...
            for (int i = 0; i < numberofships; i++)
            {
                if (balls[ballnumber].visible && spaceship[i].visible && spaceship[i].spawntime > lengthofspawn && balls[ballnumber].bsphere.Intersects(spaceship[i].bsphere))
                {
                    ...
                    vibration = 1000f;
                    shiphit[i].start(balls[ballnumber].position, spaceship[i].rotation, 0);
                }
            }

            for (int counter = 0; counter < ballsinplay; counter++)
        ...
        {
            if (bullet[ship1].visible && spaceship[ship2].visible && bullet[ship1].bsphere.Intersects(spaceship[ship2].bsphere) && spaceship[ship2].spawntime > lengthofspawn)
            {
                ...
                shiphit[ship2].start(spaceship[ship2].position, spaceship[ship2].rotation, 0);
                spaceship[ship2].spawntime = 0;
            }
        }
        ...
        void bulletast(int astnum, int ship)
        {
            for (int i = 0; i < ballsinplay; i++)
            {
                if (spaceship[ship].visible && bullet[ship].visible && bullet[ship].bsphere.Intersects(balls[i].bsphere) && bullet[ship].visible && balls[i].visible)
                {
                    ...
                    explosion[i].start(balls[i].position, bullet[ship].rotation, 0);
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (!gameover)
            {
                ...
                for (int i = 0; i < numberofships; i++)
                {
                    ...
                    spaceship[i].moveme(pad[i], backgscroll.gamewidth, backgscroll.gameheight, timebetweenupdates);
                }
                ...
                for (int i = 0; i < ballsinplay; i++)
                {
                    balls[i].automove(backgscroll.gamewidth, backgscroll.gameheight, timebetweenupdates);
                    int ballhit = checkcollision(i);
                    if (ballhit > -1)
                    {
                        // The line bleow has problems and is to loud
                       // collidesound.Play();
                    }
                }
                ...
                for (int i = 0; i < numberofships; i++)
                    shiphit[i].update(timebetweenupdates);
